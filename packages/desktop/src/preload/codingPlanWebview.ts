    ...
    func: (
      ...
      const bridge = {
        ...
        getLang() {
          ...
          return value === "zh-CN" || value === "en-US" || value === "es-ES" ? value : null;
        },
        getReportContext() {
        ...
        onLangChange(callback: (locale: "zh-CN" | "en-US" | "es-ES") => void) {
          const handler = (event: Event) => {
            const detail = (event as CustomEvent<{ locale?: unknown }>).detail;
            if (
              detail &&
              (detail.locale === "zh-CN" || detail.locale === "en-US" || detail.locale === "es-ES")
            ) {
