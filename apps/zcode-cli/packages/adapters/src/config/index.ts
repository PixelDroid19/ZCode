...
import { getConfigDefaultValue, hasConfigDefaultValue } from "./config-defaults.js";
...
export class ConfigPortImpl implements ConfigPort {
  ...
  get<K extends ConfigKey>(key: K): ConfigValue<K> {
    ...
    const defaultValue = getConfigDefaultValue(key) as ConfigValue<K> | undefined;
  ...
  has(key: ConfigKey): boolean {
    return this.store.has(key) || hasConfigDefaultValue(key);
  }
